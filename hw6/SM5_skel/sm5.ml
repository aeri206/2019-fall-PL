...
struct
  ...
  *)
  ...
  and command_to_str indent comm = 
    indent ^ "[" ^ 
    (String.concat ("; " ^ indent) (List.map (cmd_to_str indent) comm)) ^ "]"
  ...
  let malloc_with_gc s m e c k =
    if List.length m < mem_limit then
      ...
      let rec check_sv sv =
        match sv with
          | V v -> check_v v
          | P (_, _, env) -> check_env env
          | M ev -> check_ev ev
      and check_v v =
        match v with
          | L l -> add_loc l
          | R r -> List.iter check_rec r
          | _ -> ()
      and check_rec (_, loc) = add_loc loc
      and check_ev env =
        match env with
          | (_, Loc loc) -> add_loc loc
          | (_, Proc proc) -> check_proc proc
      and check_proc (_, _, env) = List.iter check_ev env
      and check_env env = List.iter check_ev env
      and check_k cont = List.iter (fun (_, env) -> check_env env) cont
      and add_loc l = begin
        if (List.mem l !reachable_locs) then ()
        else (
          let _ = reachable_locs := l :: (!reachable_locs) in
          let same_base = List.filter (fun (loc, _) -> (fst loc) = (fst l)) m in
          let same_base_loc = List.map fst same_base in
          let _ = List.iter add_loc same_base_loc in
          List.iter (fun loc -> (let v = load loc m in check_v v) ) same_base_loc
        )
        end in
      let _ = List.iter check_sv s in (* s check *)
      let _ = check_env e in (* e check *)
      let _ = check_k k in (* k check *)
